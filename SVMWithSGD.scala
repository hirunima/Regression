...
object SVMWithSGD {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val renamed_data =new_data.withColumn("DOJ", new_data("DOJ").cast(DateType))
      ...
      .withColumn("label",new_data( "label").cast(DoubleType))
    ...
    val featureIndexer = new VectorIndexer()
      ...
      //      .setMaxCategories(4)
      .fit(data)
    val FinalFeatures = featureIndexer.transform(labelIndexer)
    FinalFeatures.show(20)
    // Split the data into training and test sets (20% held out for testing).
    val final_data=FinalFeatures.select("indexedLabel","features","label")
    val Array(trainingData_org, testData) =final_data.randomSplit(Array(0.7, 0.3))
...
//    testData.groupBy("label").agg(count("label")).show()
    ...
    predictions.show()
    val final_table=predictions.withColumn("prediction", predictions("prediction").cast(DoubleType))
    val accuracy1 = evaluator1.evaluate(final_table)
    println(s"Accuracy = ${(accuracy1*100)}")
    println(s"Test Error = ${(1.0 - accuracy1)}")

    val predictionAndLabelsqw =final_table.select("indexedLabel","prediction").rdd
