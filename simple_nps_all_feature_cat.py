...
OUTPUT_PATH = PATH_TO_DRIVE_ML_DATA+"/outputs/model"
...
#     ,pd.get_dummies(df_data['device_brand'],prefix='device_brand')
    ...
    df_resampled.drop('sentiment',axis=1)
    df_test.drop('sentiment',axis=1)
...
def prediction_model(x_train,label_col):
...
model=prediction_model(x_train,label_col)
log = callbacks.CSVLogger(OUTPUT_PATH + '/log.csv')
checkpoint = callbacks.ModelCheckpoint(OUTPUT_PATH + '/weights-{epoch:02d}.h5', monitor='val_acc',
